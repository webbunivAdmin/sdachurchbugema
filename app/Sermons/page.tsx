    ...
    <main className="min-h-screen p-8">
      ...
      <div className="flex justify-center gap-4 mb-6, mt-4">
        <Button
          ...
          className={activeTab === "documents" ? "#2F557F text-white" : "bg-gray-300"}
        >
          Documents
        </Button>
        <Button
          onClick={() => setActiveTab("videos")}
          className={activeTab === "videos" ? "#2F557F text-white" : "bg-gray-300"}
        >
          Videos
        </Button>
      </div>
      ...
      {activeTab === "documents" ? (
        <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 mt-4">
          {sermons.map((sermon, index) => (
            <div key={index} className="p-4 border rounded-lg shadow-lg">
              ...
              <Link href={sermon.file} target="_blank" rel="noopener noreferrer">
        ...
        <section className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mt-4">
          {videos.map((video, index) => (
            <VideoCard key={index} title={video.title} image={video.image} date={video.date} videoId={video.videoId} />
          ))}
        </section>
      )}
    </main>
